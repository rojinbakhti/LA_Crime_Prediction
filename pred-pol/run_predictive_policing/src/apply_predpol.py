...
# 'output/predpol_drug_predictions'
output_location_predictions = args.predictions
output_location_observed = args.observed  # 'output/predpol_drug_observed'

if add_crimes_logical:
    output_location_predictions += '_add_' + \
        str(int(percent_increase*100)) + 'percent.csv'
else:
    output_location_predictions += '.csv'

if add_crimes_logical:
    output_location_observed += '_add_' + \
        str(int(percent_increase*100)) + 'percent.csv'
else:
...
for i in range(num_predictions):
    ...
    if add_crimes_logical and i >= begin_predpol:
        ...
        add_crimes = np.random.binomial(
            list(crime_today_predicted + 1), percent_increase)
        new_crimes = pd.DataFrame()
        new_crimes['bin'] = np.repeat(
            list(crime_today_predicted.index), add_crimes)
        new_crimes['DateTime'] = end_date
        new_crimes['OCCURRED'] = end_date
        new_crimes['LAG'] = 0
        new_crimes.index = range(
            1 + max(data.index), 1 + max(data.index) + sum(add_crimes))
